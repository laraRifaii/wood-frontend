...
function formatSubtitle(text: string, charsPerLine = 24): React.ReactNode {
  const words = text.split(' ');
  const lines: string[] = [];
  let current = '';

  for (const word of words) {
    if ((current + ' ' + word).trim().length > charsPerLine && current.length > 0) {
...
export default function HeroSection({
  ...
  return (
    <section
      className="w-full relative min-h-[64vh] md:min-h-[100vh]  flex items-center justify-center bg-cover bg-left bg-no-repeat overflow-visible z-0"
      style={{ backgroundImage: `url('${getImageUrl(backgroundImage)}')` }}
    >
      <div className="absolute inset-0 bg-[#343536]/60 " />

      <div className="absolute right-0 top-0 bottom-0 w-3/12  hidden lg:block">
        <div
      ...
      <div className="absolute right-0 top-30  md:top-25 w-10/12 z-10">
        ...
        <div className="relative grid lg:grid-cols-[70%_30%] md:grid-cols-[70%_30%] gap-4 pt-6 md:p-9 h-full">
          <div className="pl-6 pr-13 pb-6 border-b border-white md:border-b-0 md:border-r text-white flex flex-col justify-center">
            ...
            <p className="text-sm md:text-[20px] py-4 text-[#FFDBBB] md:pt-10">{formatSubtitle(subtitle)}</p>
            <div className="pt-4">
