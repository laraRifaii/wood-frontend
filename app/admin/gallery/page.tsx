...
export default function GalleryAdminPage() {
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      setUploading(false); // always reset button
    }
  };
  ...
  const updateMeta = (id: string, key: "alt" | "category", val: string) => {
    setImages((imgs) =>
      imgs.map((i) => (i.id === id ? { ...i, [key]: val } : i)),
    );
  };
  ...
  return (
    <div className="p-8">
      ...
      {/* Drop zone */}
      <div
        className={`border-2 border-dashed p-10 text-center mb-6 cursor-pointer transition-colors rounded-sm ${
          uploading
            ? "border-wood-ember/50 bg-wood-ember/5"
            : "border-wood-deep/25 bg-charcoal hover:border-wood-ember/50"
        }`}
        onClick={() => !uploading && fileRef.current?.click()}
        onDragOver={(e) => {
          e.preventDefault();
          e.currentTarget.style.borderColor = "var(--color-wood-ember)";
        }}
        onDragLeave={(e) => {
      ...
      >
        {uploading ? (
          <>
            <div className="w-5 h-5 border-2 border-wood-ember border-t-transparent rounded-full animate-spin mx-auto mb-2" />
            <p className="text-sm text-wood-ember font-inter">Uploading...</p>
          </>
        ) : (
          <>
            ...
            <p className="text-sm text-taupe font-inter">
              Drop images here or{" "}
              <span className="text-wood-ember">click to upload</span>
            </p>
          ...
          {images.map((img, idx) => (
            ...
            >
              ...
              >
                ...
                {/* Order badge */}
                <div
                  className="absolute top-2 left-2 w-5 h-5 flex items-center justify-center bg-obsidian/80 text-wood-ember rounded-sm"
              ...
              <div className="p-2 flex flex-col gap-1.5">
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateMeta(img.id, "category", e.target.value)
                  }
