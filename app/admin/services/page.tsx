...
export default function ServicesAdminPage() {
  ...
  const handleServiceImageFile = async (file: File) => {
  ...
  const EditForm = () => (
    <div className="mt-1 p-5 border border-wood-ember/20 bg-obsidian rounded-sm">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <Field
          label="Icon name"
          hint="e.g. sofa, home, wrench (Lucide icon name)"
        >
          <Input
            value={form.icon}
      ...
      <div className="mb-5">
        <Field label="Service image">
          <ThumbUpload
            value={form.image ? getImageUrl(form.image) : undefined}
    ...
    <div className="p-8">
      ...
      <div className="mb-6 px-4 py-3 border border-steel-dark/30 bg-steel-dark/10 rounded-sm">
        <p className="text-xs text-steel-light font-inter leading-relaxed">
          <span className="font-medium text-steel-light">💡 Suggestion:</span>{" "}
          Services are best displayed in a dedicated section between the gallery
          and advantages — e.g. "What we offer" with icon cards. Add a{" "}
          <code className="bg-obsidian px-1 rounded">ServicesSection</code>{" "}
          component to your homepage and pass the services from the API.
        </p>
      </div>
            ...
            <div key={service.id} className="rounded-sm overflow-hidden">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-ash font-inter mb-0.5">
                    {service.title}
                  </p>
                  <p className="text-xs text-slate font-inter truncate">
                    {service.description}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button
                    onClick={() =>
                      editingId === service.id ? cancel() : startEdit(service)
                    }
